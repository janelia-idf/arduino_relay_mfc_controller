...
class ArduinoRelayMfcController(ArduinoDevice):
    ...
    def __init__(self,*args,**kwargs):
        ...
        if ('port' not in kwargs) or (kwargs['port'] is None):
            port =  findArduinoRelayMfcControllerPort(baudrate=kwargs['baudrate'],
                                                      serial_number=serial_number)
            kwargs.update({'port': port})
        super(ArduinoRelayMfcController,self).__init__(*args,**kwargs)
        atexit.register(self._exitArduinoRelayMfcController)
        dev_info = self.getDevInfo()
        self.relay_count = dev_info['relay_count']
        self.mfc_count = dev_info['mfc_count']

    def _exitArduinoRelayMfcController(self):
        try:
            self.stopAllRelaysBlink()
        except IOError:
            pass

    def getArduinoRelayMfcControllerInfo(self):
...
class ArduinoRelayMfcControllers(ArduinoDevices):

    def __init__(self,*args,**kwargs):
        if ('use_ports' not in kwargs) or (kwargs['use_ports'] is None):
            kwargs['use_ports'] = findArduinoRelayMfcControllerPorts(*args,**kwargs)
        super(ArduinoRelayMfcControllers,self).__init__(*args,**kwargs)

    def appendDevice(self,*args,**kwargs):
        self.append(ArduinoRelayMfcController(*args,**kwargs))

    def getArduinoRelayMfcControllerInfo(self):
        arduino_relay_mfc_controller_info = []
        for device_index in range(len(self)):
            dev = self[device_index]
            arduino_relay_mfc_controller_info.append(dev.getArduinoRelayMfcControllerInfo())
        return arduino_relay_mfc_controller_info


def findArduinoRelayMfcControllerPorts(baudrate=None, serial_number=None, try_ports=None, debug=DEBUG):
...
def findArduinoRelayMfcControllerPort(baudrate=None, serial_number=None, try_ports=None, debug=DEBUG):
    arduino_relay_mfc_controller_ports = findArduinoRelayMfcControllerPorts(baudrate=baudrate,
                                                                            serial_number=serial_number,
                                                                            try_ports=try_ports,
                                                                            debug=debug)
...
def isArduinoRelayMfcControllerPortInfo(port_info):
    return port_info['model_number'] == DEVICE_MODEL_NUMBER

def arduinoRelayMfcControllerCli():
    ...
    o = ArduinoRelayMfcController(debug=DEBUG)
    # if not args.debug:
    #     o = ArduinoRelayMfcController()
    # else:
    #     o = ArduinoRelayMfcControllerDebug()
    ...
    if args.subparser_name == 'relay':
        ...
        print(o.getArduinoRelayMfcControllerInfoDict())
